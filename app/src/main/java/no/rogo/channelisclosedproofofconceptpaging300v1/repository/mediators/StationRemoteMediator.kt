...
class StationRemoteMediator(
    ...
    ): MediatorResult
    {
        ...
        Log.i(TAG, "load:() state.pages.lastOrNull()?.data?.size = ${state.pages.lastOrNull()?.data?.size}")
        if(state.pages.lastOrNull()?.data?.isNotEmpty() == true)
            Log.i(TAG, "load:() state.pages.lastOrNull()?.data = ${state.pages.lastOrNull()?.data}")
        Log.i(TAG, "load:() state.pages.lastOrNull()?.prevKey = ${state.pages.lastOrNull()?.prevKey}")
        Log.i(TAG, "load:() state.pages.lastOrNull()?.nextKey = ${state.pages.lastOrNull()?.nextKey}")
        Log.i(TAG, "load:() state.pages.lastOrNull()?.itemsAfter = ${state.pages.lastOrNull()?.itemsAfter}")
        Log.i(TAG, "load:() state.pages.lastOrNull()?.itemsBefore = ${state.pages.lastOrNull()?.itemsBefore}")
